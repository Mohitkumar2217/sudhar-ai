      ...
      <div className="flex items-stretch">
        {STAGES.map((stage, i) => (
          <div key={stage.key} className="flex items-stretch flex-1">
            <div className="group flex flex-col items-center text-center flex-1 px-2 py-2 rounded-md transition-all duration-200 hover:-translate-y-1 hover:bg-bg hover:shadow-md">
              <span className={`font-mono text-2xl md:text-3xl font-medium ${STAGE_TONE[stage.key]}`}>
                {counts[stage.key] ?? 0}
              </span>
              <span className="font-display text-[11px] uppercase tracking-[0.1em] text-muted mt-1 transition-colors group-hover:text-ink">
                {stage.label}
              </span>
            </div>
            {i < STAGES.length - 1 && (
